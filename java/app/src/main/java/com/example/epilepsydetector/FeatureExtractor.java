...
public class FeatureExtractor {
    ...
    // Classifier
    //Call weightedkNN and it calculates the class for the value in featureValues. 
    // I'm not sure where it should be called or if it's placed the right place. - Victor
    public int weightedkNN(){
        ...
        // mean is the mean of the linear phase (data point)
        double[] mean = new double[]{1.1726,1.4699,1.1850,1.3688,1.3176,1.4110,1.2727,1.1755,1.4459,1.3521,1.1167,1.2245};
        ...
        for (int i = 0; i < n; i++){
            dis[i] = (Math.sqrt((Mobi_coor[i]- featureValues[1]) * (Mobi_coor[i] - featureValues[1]) +
                    ...
                    (mean[i] - featureValues[6]) * (mean[i] - featureValues[6])));
        }
